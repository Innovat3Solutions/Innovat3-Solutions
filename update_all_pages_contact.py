...
def update_service_pages():
    """Update all service pages with contact section."""
    ...
    for service_file in service_dir.glob("*.html"):
        try:
            ...
            # Add contact section before footer if it doesn't exist
            if 'unified-contact-section' not in content:
                contact_html = generate_contact_html("service")

                # Insert before footer
                pattern = r'(<footer class="footer">)'
...
def update_industry_pages():
    """Update all industry pages with contact section."""
    ...
    for industry_file in industry_dir.glob("*.html"):
        try:
            ...
            # Add contact section before footer if it doesn't exist
            if 'unified-contact-section' not in content:
                contact_html = generate_contact_html("industry")
...
def update_blog_posts():
    """Update all blog posts with contact section."""
    ...
    for blog_file in blog_dir.glob("*.html"):
        ...
        try:
            ...
            # Add contact section after sources section if it doesn't exist
            if 'unified-contact-section' not in content:
                contact_html = generate_contact_html("blog")

                # Insert after the sources section, before the closing scripts
                pattern = r'(</div>\s*</div>\s*<script src="../../../js/main.js"></script>)'
                if re.search(pattern, content):
                    content = re.sub(
                        pattern,
...
def main():
    ...
    print("=" * 80)
    print()
    print(f"Total pages updated:")
    print(f"  - Service pages: {service_count}")
    print(f"  - Industry pages: {industry_count}")
    print(f"  - Blog posts: {blog_count}")
    print(f"  - Total: {service_count + industry_count + blog_count}")
    print()
